...
public class TriangleRemoval: HoloToolkit.Unity.Singleton<TriangleRemoval>
{
  ...
  public void RemoveSurfaceVerticesWithinBounds(IEnumerable<GameObject> boundingObjects)
  {
    ...
    m_total_triangles = 0;
    foreach (MeshFilter filter in HoloToolkit.Unity.SpatialMapping.SpatialMappingManager.Instance.GetMeshFilters())
    {
      m_total_triangles += (filter.sharedMesh.triangles.Length / 3);
    }
