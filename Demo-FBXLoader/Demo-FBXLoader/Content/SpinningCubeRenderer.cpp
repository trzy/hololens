...
void SpinningCubeRenderer::CreateDeviceDependentResources()
{
    ...
    {
        // Load mesh
        FBXLoader fbx("Assets/heli.fbx");
        std::vector<VertexPositionColor> vertices;
        int color = 0;
        for (auto &vertex : fbx.GetVertices())
        {
          vertices.push_back({ vertex, XMFLOAT3((color & 4) ? 1.0f : 0.0f, (color & 2) ? 1.0f : 0.0f, (color & 1) ? 1.0f : 0.0f) });
          vertices.back().pos.x *= .01f;
          vertices.back().pos.y *= .01f;
          vertices.back().pos.z *= .01f;
          ++color;
        }

        // Create vertex buffer
        D3D11_SUBRESOURCE_DATA vertexBufferData = {0};
        vertexBufferData.pSysMem = vertices.data(); //cubeVertices;
        vertexBufferData.SysMemPitch = 0;
        vertexBufferData.SysMemSlicePitch = 0;
        const CD3D11_BUFFER_DESC vertexBufferDesc(vertices.size() * sizeof(VertexPositionColor), D3D11_BIND_VERTEX_BUFFER);
        DX::ThrowIfFailed(
            m_deviceResources->GetD3DDevice()->CreateBuffer(
                &vertexBufferDesc,
                &vertexBufferData,
        ...
        // Create index buffer
        m_indexCount = fbx.GetIndices().size(); // indices.size();
        D3D11_SUBRESOURCE_DATA indexBufferData = {0};
        indexBufferData.pSysMem          = fbx.GetIndices().data();//cubeIndices;
        indexBufferData.SysMemPitch      = 0;
        indexBufferData.SysMemSlicePitch = 0;
        const CD3D11_BUFFER_DESC indexBufferDesc(fbx.GetIndices().size()*sizeof(unsigned short)/*sizeof(cubeIndices)*/, D3D11_BIND_INDEX_BUFFER);
        DX::ThrowIfFailed(
            m_deviceResources->GetD3DDevice()->CreateBuffer(
                &indexBufferDesc,
                &indexBufferData,
